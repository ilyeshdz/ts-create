...
export async function compileBlueprint(sourcePath: string): Promise<BlueprintResult> {
  ...
  const contentFiles: BlueprintFile[] = [];
  for (const [relPath, ext] of externalContents) {
    contentFiles.push({
...
`;
  ...
  const files: BlueprintFile[] = [{ path: "generator.ts", content: mainCode }, ...contentFiles];
